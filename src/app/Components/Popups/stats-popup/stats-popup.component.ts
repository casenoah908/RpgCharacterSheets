...
import { BackgroundSkillsComponent } from '../background-skills/background-skills.component';
...
export class StatsPopupComponent implements OnInit {
  ...
  formIncomplete: boolean;
    ...
    //background skills popup flag
    backgroundSkillsNeeded: boolean;
    //background skill proficiency choices list
    backgroundProfChoices: Array<string>;
  ...
  ngOnInit(): void {
    ...
    this.backgroundSkillsNeeded = false;
  }
  ...
  openSkillsPopup() {
    ...
    if (this.strength < 3 || this.strength > 18 || this.dexterity < 3 || this.dexterity > 18 || this.constitution < 3 || this.constitution > 18 || this.intelligence < 3 || this.intelligence > 18 || this.wisdom < 3 || this.wisdom > 18 || this.charisma < 3 || this.charisma > 18) {
      ...
      this.character.setCharisma(this.charisma);
       ...
       //if background is one where you have to choose from a list of proficincies, open background skills popup
      //determine backgrondSkillsNeeded and backgroundProfs
      if (this.character.getBackground() == 'Custom') {
        this.backgroundSkillsNeeded = true;
        //all 18 skills for custom
        this.backgroundProfChoices = ['2', 'Acrobatics', 'Animal Handling', 'Arcana', 'Athletics', 'Deception', 'History', 'Insight', 'Intimidation', 'Investigation', 'Medicine', 'Nature', 'Perception', 'Performance', 'Persuasion', 'Religion', 'Sleight of Hand', 'Stealth', 'Survival'];
      }
      else if (this.character.getBackground() == 'Cloistered Scholar') {
        this.backgroundSkillsNeeded = true;
        this.backgroundProfChoices = ['2', 'Arcana', 'Nature', 'Religion'];
      }
      else if (this.character.getBackground() == 'Faction Agent') {
        this.backgroundSkillsNeeded = true;
        this.backgroundProfChoices = ['1', 'Animal Handling', 'Arcana', 'Deception', 'History', 'Insight', 'Intimidation', 'Investigation', 'Medicine', 'Nature', 'Perception', 'Performance', 'Persuasion', 'Religion', 'Survival'];
      }
      else if (this.character.getBackground() == 'Haunted One') {
        this.backgroundSkillsNeeded = true;
        this.backgroundProfChoices = ['2', 'Arcana', 'Investigation', 'Religion', 'Survival'];
      }
      else if (this.character.getBackground() == 'Inheritor') {
        this.backgroundSkillsNeeded = true;
        this.backgroundProfChoices = ['1', 'Arcana', 'History', 'Religion'];
      }
      else if (this.character.getBackground() == 'Knight of the Order') {
        this.backgroundSkillsNeeded = true;
        this.backgroundProfChoices = ['1', 'Arcana', 'History', 'Nature', 'Religion'];
      }
      else if (this.character.getBackground() == 'Urban Bounty Hunter') {
        this.backgroundSkillsNeeded = true;
        this.backgroundProfChoices = ['2', 'Deception', 'Insight', 'Persuasion', 'Stealth'];
      }


      //if background demands choices be made... move to backgroundSkillsComponent
      if (this.backgroundSkillsNeeded) {
        //open next popup
       let dialogRef = this.dialog.open(BackgroundSkillsComponent, {
        // size of popup
        width: '600px',
        ...
        data: {
          ...
          dialogBackgroundInfo: this.backgroundProfChoices //proficiency choices for background skills component
        }
      });
      this.thisDialogRef.close();
      } 

      //if background provides all proficiencies, move to skills component
      else{
      //open next popup
